...
**联合索引**：一个组合索引包含两个或两个以上的列，查询的时候遵循 mysql 组合索引的最左前缀原则
...
| 页目录(Page Directory)         | 不确定   | 储存用户记录的相对位置           |
