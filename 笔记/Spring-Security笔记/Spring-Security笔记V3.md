...
如果不编写配置类会有很对默认行为，<a href="#默认行为">详见</a>

```java
// 旧版本使用 and 连接，新版 SS 推荐使用 lambda
// 旧版本：通过扩展 WebSecurityConfigurerAdapter 类并重写其 configure 方法来配置 Spring Security
// 使用 http.authorizeRequests() 方法链式调用来配置请求授权
@Configuration
...
/////////////////////////////////////////////////////////////////////
// 新版本
// 新版本（5.7+）: WebSecurityConfigurerAdapter 已被弃用，推荐直接使用 SecurityFilterChain Bean 来配置安全过滤链
// 使用 http.securityMatcher 和 authorizeHttpRequests 方法来替代以前的 authorizeRequests
@Configuration
@EnableWebSecurity
...
- 默认账号：user，默认密码由项目启动时自动生成，输出在控制台
...
- 提供基于表单的登录和注销流程
- 对基于表单的登录以及 HTTP Basic 进行认证
- 提供内容协商，对于web请求，重定向到登录页面，对于服务请求，返回 401 Unauthorized
- 防御 CSRF 攻击
- 防御 Session Fixation 攻击
- 写入 Strict-Transport-Security，确保 HTTPS
- 写入 X-Content-Type-Options 以防御嗅探攻击
- 写入保护认证资源的 Cache Control header
- 写入 X-Frame-Options，以防御点击劫持的情况
- 与 HttpServletRequest 的认证方法整合
- 发布认证成功和失败的事件
...
Spring Security 采用的是**责任链**的设计模式，它有一条 15 个过滤的过滤器链：在启动时打断点可以获取到过滤器链

- **WebAsyncManagerIntegrationFilter**：将 Security 上下文与 Spring Web 中用于处理异步请求映射的 **WebAsyncManager** 进行集成
- **SecurityContextPersistenceFilter**：在**每次请求处理之前**将该请求相关的安全上下文信息加载到 **SecurityContextHolder** 中，然后在该次请求处理完成之后，将 SecurityContextHolder 中关于这次请求的信息存储一份，然后将 SecurityContextHolder 中的信息清除，例如在 Session 中维护一个用户的安全信息就是这个过滤器处理的
- **HeaderWriterFilter**：用于将头信息加入响应中
- **CsrfFilter**：用于处理跨站请求伪造
- **LogoutFilter**：用于处理退出登录
- **UsernamePasswordAuthenticationFilter**：用于处理基于表单的登录请求，从表单中获取用户名和密码默认情况下处理来自 /login 的请求从表单中获取用户名和密码时，默认使用的表单 name 值为 username 和 password，这两个值可以通过设置这个过滤器的 **usernameParameter** 和 **passwordParameter** 两个参数的值进行修改
...
-  **RememberMeAuthenticationFilter**：当用户没有登录而直接访问资源时，从 cookie  里找出用户的信息，如果 Spring Security 能够识别出用户提供的 remember me cookie，用户将不必填写用户名和密码，而是直接登录进入，该过滤器默认不开启
...
DelegatingFilterProxy 是 Spring 提供的 Filter 实现，通过它连接 Servlet 容器和 Spring 的 ApplicationContext

DelegatingFilterProxy 的存在是因为，Servlet 容器只允许通过自身标准注册的 Filter 实例，其不知道 Spring 注册的 Filter Bean，而 DelegatingFilterProxy 可以被注册到 Servlet 容器中，并且可以将工作委托给实现了 Filter 的 Spring Bean

DelegatingFilterProxy 还可以具备懒加载，因为 Servlet 容器注册 Filter 早于 Spring 使用 ContextLoaderListener 加载 Bean

<img src="./images/image-20240810113411720.png" alt="image-20240810113411720" style="zoom:50%;" />



## 3、FilterChainProxy

FilterChainProxy 是 Spring Security 提供的一个特殊的 javax.servlet.Filter Filter，是整个 Spring Security 过滤器链的入口点，管理多个安全过滤器，并根据请求的路径或其他规则，将请求路由到适当的过滤器链进行处理

FilterChainProxy 内部维护了一个或多个 SecurityFilterChain，每个 SecurityFilterChain 包含一组过滤器（Filter 实例），这些过滤器共同协作，处理各种安全相关的任务
...
public class FilterChainProxy extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest request, 
        ...
        FilterChain chainToUse = getFilterChainForRequest(request);
...
<img src="./images/image-20240810114329761.png" alt="image-20240810114329761" style="zoom:50%;" />
...
SecurityFilterChain 被 FilterChainProxy 用来确定当前请求应该调用哪些 Security Filter 实例

SecurityFilterChain 是 Spring Security 中用于定义一组安全过滤器的链，这些 Security Filter 通常被定义为 Spring Bean

SecurityFilterChain 通过 FilterChainProxy 统一注册和管理带来了以下优点：

- 在进行故障诊断或调试时，FilterChainProxy 是一个起点，因为它可以捕获和处理所有进入安全过滤器链的请求
- 较好的灵活性，使用 RequestMatcher 接口，根据 HttpServletRequest 中的任意内容来决定是否调用 SecurityFilterChain
- 防止 SecurityContext 造成的内存泄漏
- 防护恶意攻击

<img src="./images/image-20240810115810710.png" alt="image-20240810115810710" style="zoom:50%;" />



### 2、多实例

在 Spring Security 中，FilterChainProxy 可以管理多个 SecurityFilterChain 实例，每个 SecurityFilterChain 定义了一组与请求匹配的过滤器链，并负责处理特定类型的请求

在请求中，FilterChainProxy 决定应该使用哪个 SecurityFilterChain，并且只有第一个匹配的 SecurityFilterChain 被调用

<img src="./images/image-20240810121009130.png" alt="image-20240810121009130" style="zoom:50%;" />



**注意**：

- 零 Filter 实例的 SecurityFilterChain 将会使 Spring Security 忽略该请求，不做任何处理的转给 Servlet
...
通过实现 SecurityFilterChain 接口，可以将特定的过滤器链配置到 FilterChainProxy 中

Filter 在处理请求时是按照特定的顺序执行的，这是为了确保过滤器在正确的时机被调用，例如先认证在访问

查看 FilterOrderRegistration 了解 Filter 顺序
...
- 如果使用 @Component 或者 @Bean 修饰自定义的 Filter，Spring Boot 会自动在嵌入式容器中注册它，如果同时使用了 HttpSecurity 的 addFilter 等函数添加过滤器，这会导致导致 Filter 被调用两次，一次由 Servlet 容器调用，一次由 Spring Security 调用，而且顺序不同
  - 要处理这种情况可以使用 FilterRegistrationBean，将其 Enabled 设置为 false，拒绝注入到 Servlet 容器
  - 或者使用 OncePerRequestFilter 类
...
**异常过滤器**，用来处理在认证授权过程中抛出的异常，允许将 AccessDeniedException 和 AuthenticationException 转换为 HTTP 响应

<img src="images/d3ce9f84dd2343a2b2f8f86f77a97d15.png" alt="d3ce9f84dd2343a2b2f8f86f77a97d15" style="zoom:50%;" />
...
public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
    ...
    try {
        ...
        // 如果出现异常，判断异常类型
        // 如果是 AuthenticationException，则开始身份验证
        // 		清除 SecurityContextHolder 的身份验证
        //		（SEC-112：清除 SecurityContextHolder 的身份验证，因为现有身份验证不再有效）
        //		将 HttpServletRequest 保存在 RequestCache 中
        //		当用户成功进行身份验证时，RequestCache 用于重现原始请求
        //		AuthenticationEntryPoint 用于从客户端请求凭据
        //		例如，它可能会重定向到登录页面或发送 WWW-Authenticate 标头
        // 如果是 AccessDeniedException
        // 		拒绝访问，调用 AccessDeniedHandler 来处理拒绝的访问
...
-  如果用户没有被认证，或者是一个 AuthenticationException，那么就开始认证
  ...
  - AuthenticationEntryPoint 用于请求客户的凭证，例如重定向到一个登录页面或发送一个 WWW-Authenticate 头
...
- 如果程序没有抛出 AccessDeniedException 或 AuthenticationException 异常，那么 ExceptionTranslationFilter 不会做任何响应
...
当用户提交凭证时，AbstractAuthenticationProcessingFilter 会从 HttpServletRequest 中创建一个要认证的 Authentication，创建的认证类型取决于 AbstractAuthenticationProcessingFilter 的子类，例如 UsernamePasswordAuthenticationFilter 从 HttpServletRequest 中提交的 username 和 password 创建一个 Authentication 的实例 UsernamePasswordAuthenticationToken 传入 AuthenticationManager，以进行认证
...
- RememberMeServices.loginFail 被调用，如果没有配置记住我（remember me），则不做什么处理
...
- Authentication 是在 SecurityContextHolder 上设置的，如果需要保存 SecurityContext 以便在未来的请求中自动设置，必须显式调用 SecurityContextRepository#saveContext

- RememberMeServices.loginSuccess 被调用，如果没有配置 remember me，则不做什么处理

- ApplicationEventPublisher 发布一个 InteractiveAuthenticationSuccessEvent 事件

- AuthenticationSuccessHandler 被调用，通常是 SimpleUrlAuthenticationSuccessHandler 实例，重定向到登录页面时，它会重定向到由 ExceptionTranslationFilter 保存的请求

<img src="./images/image-20240812114643884.png" alt="image-20240812114643884" style="zoom:50%;" />



#### 4、UsernamePasswordAuthenticationFilter

对登录路径的 **POST** 请求做拦截，校验表单中用户名，密码
...
与 SecurityContextHolderFilter 互斥

默认不添加该过滤器，需要手动配置

<img src="./images/image-20240815153716952.png" alt="image-20240815153716952" style="zoom:50%;" />
...
public void doFilter(ServletRequest req, 
    ...
    HttpRequestResponseHolder holder = new HttpRequestResponseHolder(request, response);
   	// 当请求到来时从 SecurityContextRepository 加载 SecurityContext
    // repo 加载 SecurityContext 对象，如果有则取出，没有则创建
   	// 由于 repo 通常是 HttpSessionSecurityContextRepository，因此从 Session 中加载
    SecurityContext contextBeforeChainExecution = repo.loadContext(holder);
	
    try {
        // 将获取到的 SecurityContext 对象存入 SecurityContextHolder
        SecurityContextHolder.setContext(contextBeforeChainExecution);
		// 进入下一个过滤器中
        chain.doFilter(holder.getRequest(), holder.getResponse());
    }
    finally {
        // 响应返回时再次取出 SecurityContext 对象
        SecurityContext contextAfterChainExecution = SecurityContextHolder.getContext();
        // 将 SecurityContextHolder 中的 SecurityContext 对象移除
        SecurityContextHolder.clearContext();
        // 将取出的 SecurityContext 对象持久化保存，通常是保存在 Session 中
...
使用 SecurityContextRepository 在请求之间加载 SecurityContext

与 SecurityContextPersistenceFilter 不同，SecurityContextHolderFilter 只加载 SecurityContext ，不保存，如果要使用该 Filter 需要明确地保存 SecurityContext，即手动调用 repo 存储

无状态应用通常会使用

<img src="./images/image-20240815161053965.png" alt="image-20240815161053965" style="zoom:50%;" />
...
在一个请求到达时，SessionManagementFilter 首先检查 SecurityContextRepository 中是否包含当前用户的 SecurityContext

- 如果 SecurityContextRepository 已经包含一个 SecurityContext（例如：通过 remember-me 机制或者其他非交互式认证机制），那么 SessionManagementFilter 什么都不做，因为用户已经被认证
- 如果 SecurityContextRepository 中没有找到 SecurityContext，过滤器会检查 SecurityContextHolder 中的 SecurityContext，如果存在，过滤器会认为在之前的过滤器中已经完成认证

在确认用户已经被认证之后，SessionManagementFilter 会调用配置的 SessionAuthenticationStrategy，其允许自定义在用户认证之后如何管理会话

如果用户尚未认证，SessionManagementFilter 会检查当前请求是否包含无效的会话 ID

- 如果有无效会话，SessionManagementFilter 会调用配置的 InvalidSessionStrategy 来处理无效会话，其允许自定义如何处理无效会话，最常见的实现是 SimpleRedirectInvalidSessionStrategy，将用户重定向到一个固定的 URL
...
- 不太建议使用该过滤器，可以自定义去设置一些认证管理方法

| 方法                                | 替换                                   |
| :---------------------------------- | :------------------------------------- |
| sessionAuthenticationErrorUrl       | 配置一个 AuthenticationFailureHandler  |
| sessionAuthenticationFailureHandler | 配置一个 AuthenticationFailureHandler  |
| sessionAuthenticationStrategy       | 配置一个 SessionAuthenticationStrategy |
...
// 或者直接给注解 @EnableWebSecurity 添加 debug=true
~~~
...
HttpServletRequest 被保存在 RequestCache，当用户成功认证后，RequestCache 被用来重放原始请求

RequestCacheAwareFilter 就是使用 RequestCache 来保存 HttpServletRequest
...
public class SecurityConfig {
    ...
    public RequestCache requestCache() {
        // 通常使用默认的 HttpSessionRequestCache，也可以根据需要自定义
...
public class CustomAuthenticationSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler {
    ...
    protected String determineTargetUrl(HttpServletRequest request, HttpServletResponse response) {
        // 自定义登录成功后的重定向逻辑
        return requestCache.getRequest(request, response) != null ?
...
绿色部分是**认证过滤器**，需要手动配置，可以配置**多个**认证过滤器，可以自定义过滤器（例如：短信验证）

认证过滤器要注册在过滤器链中，没有注册不生效



### 2、表单认证流程

<img src="./images/image-20240813152950066.png" alt="image-20240813152950066" style="zoom:50%;" /><img src="./images/image-20240813104946317.png" alt="image-20240813104946317" style="zoom:50%;" />

认证流程是在 **UsernamePasswordAuthenticationFilter** 中处理的，具体流程如下所示：

前端提交的是一个 **POST** 方式的登录表单请求，被 **UsernamePasswordAuthenticationFilter** 拦截并在 doFilter() 方法中认证，该 doFilter() 方法的实现在其抽象父类 **AbstractAuthenticationProcessingFilter** 中，该方法主要对请求判断是否需要验证，如果需要则调用子类实现的 **attemptAuthentication()** 方法进行验证

在 attemptAuthentication() 方法中会判断该请求是否符合要求，然后生成 **Authentication** 对象再交给 **AuthenticationManager** 接口实现类进行验证

AuthenticationManager 接口实现类判断 Authentication 对象属于哪一个类型，使用对应的 **AuthenticationProvider** 对象进行验证，验证完毕后除去 Authentication 对象的敏感信息，需要实现 **credentialsContainer** 接口

如果验证成功调用 **successfulAuthentication**()方法，将认证成功的 Authentication 对象 封装入 SecurityContext 对象中并存入 SecurityContextHolder，还要发布认证成功事件等

如果失败调用 **unsuccessfulAuthentication**()方法，清除该线程在 SecurityContextHolder 中对应的 SecurityContext 对象
...
public SecurityFilterChain filterChain(HttpSecurity http) {
	http
		.formLogin(form -> form // 自定义登录页面，必须要自己提供一个登录页面
			.loginPage("/login")
			.permitAll()
		);
}
...
public SecurityFilterChain filterChain(HttpSecurity http) {
	http
		.formLogin(withDefaults()); // 使用 SS 自带的登录页面
}
~~~
...
<img src="./images/image-20240813165107403.png" alt="image-20240813165107403" style="zoom:40%;" /><img src="./images/image-20240813165258671.png" alt="image-20240813165258671" style="zoom:50%;" />

用户向其未被授权的资源 /private 发起一个未经认证的请求
...
当客户端收到 WWW-Authenticate 头时，它知道它应该用用户名和密码重试
...
SecurityContextHolder 是 Spring Security 用于存储认证用户细节，其包含了 SecurityContext

如果其内有值，说明包含了一个已经认证的用户

~~~java
// 创建一个新的 SecurityContext 实例
// SecurityContextHolder.getContext().setAuthentication(authentication)，容易造成多线程之间的竞争
SecurityContext context = SecurityContextHolder.createEmptyContext();
Authentication authentication = new TestingAuthenticationToken("username", "password", "ROLE_USER");
// SecurityContext 不关心 Authentication 的实现
...
默认情况下，SecurityContextHolder 使用 ThreadLocal 来存储，这使得 SecurityContext 在同一线程的任意方法内可以共享，即使 SecurityContext 没有被作为参数传入

<img src="./images/image-20240810155155531.png" alt="image-20240810155155531" style="zoom:50%;" />



#### 2、工作模式

SecurityContextHolder 可以工作在以下三种模式之一：

- MODE_THREADLOCAL（默认工作模式）
...
从 SecurityContextHolder 获得，包含了当前认证用户的 Authentication
...
如果作为 AuthenticationManager 的输入，用于提供用户为验证而提供的凭证，这种情况下 isAuthenticated() 返回 false

如果代表当前认证的用户，可以从 SecurityContext 中获得当前的 Authentication

Authentication 包含了：

- principal：用户信息，使用用户名密码进行认证时，通常是 UserDetails 的实例
- credentials：通常是一个密码，在许多情况下，这在用户被认证后被清除，以确保它不会被泄露
- authorities：GrantedAuthority 实例是用户被授予的权限，例如：角色（role）和作用域（scope）



### 4、GrantedAuthority

GrantedAuthority 实例是用户被授予的权限，例如：角色（role）和作用域（scope）

可以从 Authentication.getAuthorities() 法中获得 GrantedAuthority 实例，这个方法提供了一个 GrantedAuthority 对象的集合，当使用基于用户名/密码的认证时， GrantedAuthority 实例通常由 UserDetailsService 加载

每个 GrantedAuthority 对象表示一种抽象的权限，不与特定的领域对象绑定，例如不会给特定员工创建一个 GrantedAuthority

Spring Security 提供了领域对象安全（Domain Object Security），也称为 ACL（Access Control List），通过 ACL 可以针对具体的领域对象（如某个特定的员工、文件或订单）定义细粒度的权限



### 5、AuthenticationManager

AuthenticationManager 是定义 Spring Security 的 Filter 如何执行认证的 API

返回的 Authentication 是由调用 AuthenticationManager 的控制器（即 Spring Security 的 Filter 实例）在 SecurityContextHolder 上设置的，如果不与 Spring Security 的 Filter 实例集成，可以直接设置 SecurityContextHolder，不需要使用 AuthenticationManager
...
ProviderManager 是最常用的 AuthenticationManager 的实现

ProviderManager 委托给一组 AuthenticationProvider 实例，每个 AuthenticationProvider 都有机会声明认证是成功/失败，或者声明它不能决定并允许下游的 AuthenticationProvider 来决定，如果配置的 AuthenticationProvider 实例中没有一个能进行认证，那么认证就抛出 ProviderNotFoundException 后失败，这是一个特殊的 AuthenticationException，表明 ProviderManager 没有被配置为支持被传入它的 Authentication 类型

<img src="./images/image-20240812105637414.png" alt="image-20240812105637414" style="zoom:50%;" />

通常每个 AuthenticationProvider 都应对一种特定类型的认证，例如：验证一个用户名/密码，验证一个 SAML 断言
...
<img src="./images/image-20240812111454330.png" alt="image-20240812111454330" style="zoom:50%;" />

多个 ProviderManager 实例可能共享同一个父级 AuthenticationManager，这些实例之间的认证机制属于交集

<img src="./images/image-20240812111629240.png" alt="image-20240812111629240" style="zoom:50%;" />

默认情况下，ProviderManager 会清除 Authentication 中任何敏感的凭证信息，这会让缓存的用户凭证无法再次使用，可以制作用户凭证副本，或者禁用 ProviderManager 上的 eraseCredentialsAfterAuthentication 属性



### 7、AuthenticationProvider

对特定类型的 Authentication 进行校验的接口

可以在 ProviderManager 中注入多个 AuthenticationProvider 实例，每个 AuthenticationProvider 都应对一种特定类型的认证

例如 DaoAuthenticationProvider 支持基于用户名/密码的认证，而 JwtAuthenticationProvider 支持认证 JWT 令牌



### 8、DaoAuthenticationProvider

DaoAuthenticationProvider 是一个 AuthenticationProvider 的实现，使用 UserDetailsService 和 PasswordEncoder 来验证一个用户名和密码

<img src="./images/image-20240814173723749.png" alt="image-20240814173723749" style="zoom:50%;" />

读取用户名/密码的 Filter 将 UsernamePasswordAuthenticationToken 传递给 AuthenticationManager（ProviderManager ）

AuthenticationProvider 通过 support 方法确认是否可以进行校验，最后 DaoAuthenticationProvider 支持校验

DaoAuthenticationProvider 调用 UserDetailsService 的实例查找 UserDetails，然后使用 PasswordEncoder 来验证 UserDetails 上的密码

当认证成功时，返回的 Authentication 是 UsernamePasswordAuthenticationToken 类型，其内部的委托人（principal）是 UserDetailsService 实例返回的 UserDetails，最终的 UsernamePasswordAuthenticationToken 被认证 Filter 设置在 SecurityContextHolder



### 9、AuthenticationEntryPoint

AuthenticationEntryPoint 用于发送一个要求客户端提供凭证的 HTTP 响应（即重定向到登录页面，发送 WWW-Authenticate 响应）

如果客户端主动包含凭证（如用户名和密码）来请求资源，Spring Security 不会发送要求客户端提供凭证的 HTTP 响应

在其他情况下，如果客户端向未被授权访问的资源发出未经认证的请求，AuthenticationEntryPoint 实例会请求客户端提供凭证 ，AuthenticationEntryPoint 实例可能会执行重定向到一个登录页面，用 WWW-Authenticate 头来响应，或采取其他行动



### 10、UserDetailsService

当 Spring Security 使用 formLogin(Customizer.withDefaults()) 配置，账号和密码是由 Spring Security 随机生成，而在实际项目中账号和密码都是从数据库中查询出来的，需要自定义认证逻辑，需要**实现 UserDetailsService 接口**即可，接口定义如下：

```java
/**
* 加载用户特定数据的核心接口
* 它在整个框架中用作用户 DAO，并且是 DaoAuthenticationProvider 使用的策略
* 该接口只需要一种只读方法，简化对新数据访问策略的支持
*/
public interface UserDetailsService {
    /**
    * 根据用户名定位用户 
    * 在实际实现中，搜索可能区分大小写，也可能不区分大小写，具体取决于实现实例的配置方式 
    * 在这种情况下，返回的 UserDetails 对象的用户名，可能与实际请求的用户名不同
    * 参数：username – 标识需要其数据的用户的用户名
    * 返回值：完全填充的用户记录（从不为 null）
...
- 在项目启动时，会被注入进 DaoAuthenticationProvider，在 **InitializeUserDetailsBeanManagerConfigurer** 类中进行，由 getBeanOrNull(Class\<T> type) 方法根据 Bean 的类型获取



**扩展**：

- 多用户类型登陆时，可以多实现该接口，并提供 support 方法判断是否适用于此用户
...
UserDetails 由 UserDetailsService 返回，DaoAuthenticationProvider 验证 UserDetails，然后返回一个 Authentication 结果，该 Authentication 的委托人（principal）是由配置的 UserDetailsService 返回的
...
Spring Security 内具有一个 UserDetails 的实现类 **User**，之后只需要使用这个实体类即可
...
当用户发起首次请求时，由于未认证，会转到登陆页面，认证完毕后用户会被绑定到新的 Session 中以防止 Session Fixation 攻击

而之后的请求关联到用户的认证则需要 SecurityContextRepository



#### 2、SecurityContextRepository

SecurityContextRepository 的默认实现是 DelegatingSecurityContextRepository，委托给：

- HttpSessionSecurityContextRepository：将 SecurityContext 存入 Session


- RequestAttributeSecurityContextRepository：将 SecurityContext 保存为请求属性（request attribute），只在请求的生命周期内有效，而不会跨请求持久化安全上下文
- NullSecurityContextRepository：不做任何处理
...
#### 3、DelegatingSecurityContextRepository

DelegatingSecurityContextRepository 将 SecurityContext 委托到多个 SecurityContextRepository 中保存，并允许按指定顺序从任何委托（delegate）中检索
...
PasswordEncoder 接口实现类非常多，**BCryptPasswordEncoder** 是 Spring Security 推荐的密码解析器

BCryptPasswordEncoder 是对 **bcrypt** 强散列方法的具体实现，是基于 **Hash** 算法实现的单向加密，可以通过 **strength** 控制加密强度，默认强度 **10**
...
默认加载的**委派密码编码器**，内部其实是一个**Map集合**，根据传递的 **Key**（Key为加密方式）获取 Map 集合的 Value，而 Value 则是具体的 PasswordEncoder 实现类

也就是说它将具体编码的实现根据要求委派给不同的算法，以此来实现不同编码算法之间的兼容和变化协调

- 默认加载进 DaoAuthenticationProvider
...
遇到新密码，**DelegatingPasswordEncoder** 会委托给 **BCryptPasswordEncoder**（encodingId 为 bcryp）进行加密

同时，对历史上使用 ldap、MD4、MD5 等等加密算法的密码认证保持兼容（如果数据库里的密码使用的是 MD5 算法，那使用 matches 方法认证仍可以通过，但新密码会使用 bcrypt 进行储存）
...
- **encodedPassword**：是使用 encodingId 对应的密码编码器，将密码字符原序列编码后的加密字符串，假设为 xxxxx 存储的密码 
- **prefixEncodedPassword**：是在数据库中见到的形式如 {bcrypt}$2a$10$iMz8uREF/f.ChT/rpu2ZO3y



**注意**：

- 若没有在 DelegatingPasswordEncoder 中设置 defaultPasswordEncoderForMatches，数据库中的密码必须按照存储格式储存
- 若设置了 defaultPasswordEncoderForMatches，则不一定需要按照密码格式储存
...
Spring Security 的 InMemoryUserDetailsManager 可以将用户名密码存储在内存中

- 实现 **UserDetailsService** 接口，为存储在内存中的用户提供认证支持
- 实现 **UserDetailsManager** 接口，提供对 UserDetails 的
...
public class DaoUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        UserSecurity us = userRepository.findByName(username);
        if (us == null) {
            throw new UsernameNotFoundException("User not found");
        }

        return User.withUsername(us.getName())
...
public class LoginService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws 
        ...
        // 1. 放入的是角色：需要加前缀 ROLE_，而在 controller 使用时不需要加 ROLE_ 前缀，详见注解
    	// 2. 放入的是权限：不能加 ROLE_ 前缀，hasAuthority 与放入的权限名称对应即可，详见注解
    }
}
```
...
其中Header接口也继承自Map如果以第二种形式（即Map）作为参数，在setHeader()的时候会生成默认的Header接口实现**DefaultHeader**对象两种参数形式调用setHeader()，都会令Header重新赋值即：
...
在生成jwt的时候，如果不设置签名，那么header中的alg应该为nonejjwt中compact()方法实现如下：
...
- 压缩方法当载荷过长时可对其进行压缩可采用jjwt实现的两种压缩方法**CompressionCodecs.GZIP**和**CompressionCodecs.DEFLATE**

#### 8、signWith()

- 签名方法两个参数分别是签名算法和自定义的签名Key（盐）签名key可以byte[] 、String及Key的形式传入前两种形式均存入builder的keyBytes属性，后一种形式存入builder的key属性如果是第二种（及String类型）的key，则将其进行base64解码获得byte[] 
...
- 若为web授权则拦截器为 FilterSecurityInterceptor
- 若为方法授权则拦截器为 MethodSecurityInterceptor

如果同时通过web授权和方法授权则先执行 web 授权，再执行方法授权，最后决策通过，则允许访问资源，否则将禁止访问
...
public Authentication authenticate(Authentication authentication) throws AuthenticationException {
	...
	// 转换 principal 为字符串类型不过，需要 forcePrincipalAsString 参数为 true（默认为 false）
...
**注意**：此方法是 protected 类型的，子类可以重写因为，子类通常在 Authentication对象 中存储用户提供的原始凭据/密码，而非加盐、加密过的
...
针对 **ConfigAttribute#getAttribute()** 中配置为 **IS_AUTHENTICATED_FULLY、IS_AUTHENTICATED_REMEMBERED、IS_AUTHENTICATED_ANONYMOUSLY** 权限标识时的授权决策因此，其投票策略比较简单：
...
角色投票器用于 **ConfigAttribute#getAttribute()** 中配置为角色的授权决策
...
- 如果在**SavedRequest**中找到了**RequestCache**（由设置为在**ExceptionTranslationFilter**身份验证过程开始之前记录原始目标），则将重定向到该原始目标的UrlSavedRequest收到重定向的请求后，该对象将保持缓存并被拾取（请参阅参考资料SavedRequestAwareWrapper）
...
**SecurityMetadataSource** 是从数据库或者其他数据源中加载**ConfigAttribute**，为了在**AccessDecisionManager.decide()** 最终决策中进行match其有三个方法：
...
- 获取该SecurityMetadataSource对象中保存的针对所有安全对象的权限信息的集合该方法的主要目的是被**AbstractSecurityInterceptor**用于启动时校验每个**ConfigAttribute**对象
...
- **说明**：指定支持 **基于表单** 的身份验证如果没有指定{@link FormLoginConfigurer#**loginPage()**} ，将生成一个默认的登录页面
- **示例** ：
...
///强制一次只对用户的单个实例进行身份验证如果用户使用用户名 user 进行身份验证而没有注销，并且尝试再次使用 user 进行身份验证，第一个会话将被强制终止并发送到 /login?expired URL
...
- **说明**：配置 `基于容器` 的预认证在本例中，身份验证由Servlet容器管理
...
- **说明**：配置基于 `X509` 的预认证
- **示例**：这个示例将尝试从 `X509` 证书中提取用户名需要配置Servlet容器来请求客户端证书
...
- **说明**：允许配置 请求缓存 例如，一个受保护的页面 /protected 可能会在身份验证之前被请求应用程序将用户重定向到登录页面，身份验证之后，Spring Security将用户重定向到最初请求的受保护页面 /protected 当使用 {@link WebSecurityConfigurerAdapter} 时，会被默认激活
...
- **说明**：在 {@link HttpServletRequest} 之间的 {@link SecurityContext} 上建立 {@link SecurityContextHolder} 的管理当使用 {@link WebSecurityConfigurerAdapter} 时，会被默认激活

### 14、servletApi()

- **说明**：将 `{@link HttpServletRequest}` 方法与 `{@link SecurityContext}` 上的值集成起来当使用 `{@link WebSecurityConfigurerAdapter}` 时，会被默认激活

### 15、csrf()

- **说明**：添加 `CSRF` 支持
- 当使用 `{@link WebSecurityConfigurerAdapter}` 时，会被默认激活你可以禁用(disable)它

### 16、logout()

- **说明**：提供 注销 的支持当使用 {@link WebSecurityConfigurerAdapter} 时，会被默认激活默认情况下，访问URL /logout 将使HTTP会话失效，清除配置的所有 {@link HttpSecurity#rememberMe()} 身份验证，清除 {@link SecurityContextHolder} ，然后重定向到 /login?success，从而使用户退出

- **示**例：下面的配置，当 /custom-logout 接口被调用时，会走注销流程注销将删除名为 remove 的cookie，清除 SecurityContexHolder，但是不会使 HttpSession 失效，在完成上面动作之后重定向到 /logout-success
...
- **说明**：允许配置 匿名用户 的表示方式当使用 {@link WebSecurityConfigurerAdapter} 时，会被默认激活默认情况下，匿名用户将用 {@link org.springframework.security.authentication.AnonymousAuthenticationToken} ，包含角色 ROLE_ANONYMOUS
...
- **示例**2：下面演示了如何将匿名用户表示为空注意，假设启用了匿名身份验证可能会导致代码中出现空指针异常
...
- **示例**：下面的例子演示了如何为每个请求要求HTTPS不建议只支持某些请求需要HTTPS，因为允许HTTP的应用程序会引入许多安全漏洞
...
- **说明**：配置 `HTTP基本认证` 
- **示例**：下面的示例演示如何为应用程序配置HTTP基本身份验证默认的领域是 `Spring Security Application` ，但是可以使用 `{@link HttpBasicConfigurer#realmName()}` 自定义
...
- 说明：允许将 `{@link HttpSecurity}` 配置为只在匹配所提供的 `{@link RequestMatcher}` 时被调用如果需要更高级的配置，可以考虑使用`{@link #requestMatchers()}`

### 23、antMatcher()

- 说明：允许将 `{@link HttpSecurity}` 配置为只在匹配所提供的 `ant` 模式时被调用如果需要更高级的配置，可以考虑使用 `{@link #requestMatchers()}` 或 `{@link #requestMatcher()}`

### 24、mvcMatcher()

- 说明：允许将 {@link HttpSecurity} 配置为只在匹配所提供的 Spring MVC 模式时被调用如果需要更高级的配置，可以考虑使用 {@link #requestMatchers()} 或 {@link #requestMatcher(requestMatcher)} 

### 25、regexMatcher()

- 说明：允许将 `{@link HttpSecurity}` 配置为只在匹配所提供的 `正则表达式` 模式时被调用如果需要更高级的配置，可以考虑使用 `{@link #requestMatchers()}` 或 `{@link #requestMatcher()}`
...
- SharedObject是Spring Security提供的一个非常好用的功能，如果你在不同的地方需要对一个对象重复使用就可以将它注册为SharedObject，甚至直接注入Spring IoC像开头那样获取就可以了这个特性能够简化配置，提高代码的可读性，也为Spring Security的DSL特性打下了基础
